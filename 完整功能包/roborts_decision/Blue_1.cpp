    ...
    if(getbuff_act == 0)        //正式决策树
    {
        ...
        {
            //判断蓝车子弹buff在F345时 && 无子弹加成   1车抢        否则2车去
            if(blackboard->info.blue_bullet_supply == 3 || blackboard->info.blue_bullet_supply == 1 
                || blackboard->info.blue_bullet_supply == 5 || blackboard->info.blue_bullet_supply == 2)
            {
                getbuff_act = 1;
                which_buff = 4;//如果已经激活，被刷掉去下面的行为（ 射击点 ）
                if(blackboard->info.zone_array_active[blackboard->info.blue_bullet_supply] == 0)  
                    cur_state = BehaviorStateEnum::SHOOT;
            }
            else if((blackboard->info.blue_bullet_supply != 3 && blackboard->info.blue_bullet_supply != 1 
                            && blackboard->info.blue_bullet_supply != 5  && blackboard->info.blue_bullet_supply != 2) 
                            &&  blackboard->info.zone_array_active[blackboard->info.blue_bullet_supply] == 0)//2车抢到子弹后
                               cur_state = BehaviorStateEnum::SHOOT;//激进的射击点
            else  cur_state = BehaviorStateEnum::READY;//2车抢buff且还没抢到，去保守点
        }

    //    else  if(blackboard->info.remaining_time <=165 && blackboard->info.remaining_time > 150  && 
    //                  blackboard->info.zone_array_active[blackboard->info.blue_bullet_supply] == 0
    //                  &&哨岗检测到1、4象限有车){
    ...
    //后面巡逻覆盖掉165-150
        ...
        /***********************************************************************************************************************/
        else if(blackboard->info.remaining_time <150 && blackboard->info.remaining_time >= 120)//第一分钟后30秒
        {
                    //有一车状态很差或者两车都有扣血，且2车没死，去抢血量buff
            if(((blackboard->info.remain_hp_b1 <= 1500 && blackboard->info.remain_hp_b2 <= 1500) 
                || (blackboard->info.remain_hp_b1 < 1200 || blackboard->info.remain_hp_b2 < 1200))
                && (blackboard->info.remain_hp_b1 > blackboard->info.remain_hp_b2 && blackboard->info.remain_hp_b2 != 0))
            {
                getbuff_act = 1;
                which_buff = 3;
                if(blackboard->info.zone_array_active[blackboard->info.blue_hp_recovery] == 0 )  
            ...
            else if(blackboard->info.remain_hp_b1 >= 1500 && blackboard->info.remain_hp_b2 >= 1500
                        && blackboard->info.remain_bullet_b1 > 30)    
            {
                cur_state = BehaviorStateEnum::PATROL; 
                if(blackboard->info.remain_hp_b1 >= 1900 && blackboard->info.remain_hp_b2 >= 1900 
                    && blackboard->info.remain_bullet_b1> 50)
                    cur_state = BehaviorStateEnum::SHOOT; 
            }
            else if(blackboard->info.remain_bullet_b1 == 0)
                cur_state = BehaviorStateEnum::BACKBOOT;//没子弹了回家，如果子弹buff还在会被后面刷掉
            else   cur_state = BehaviorStateEnum::PATROL;//无法判断就巡逻
        }

        // else if(blackboard->info.remaining_time <125 && blackboard->info.remaining_time >= 120 
        //                 && !blackboard->info.deep_camera_is_detected
        //                 &&哨岗检测到1、4象限有车){
        //                 cur_state = BehaviorStateEnum::BACKBOOT;
        // }
        ...
        /***********************************************************************************************************************/
            else if(blackboard->info.remaining_time <100 && blackboard->info.remaining_time >= 60)
                        {
                            //如果状态不好，一车抢血量buff，子弹buff由后面大前提获取
                            if(((blackboard->info.remain_hp_b1<1800 && blackboard->info.remain_hp_b2<1800)
                                    || (blackboard->info.remain_hp_b1<800 || blackboard->info.remain_hp_b2<800))
                                    && blackboard->info.remain_hp_b1 > blackboard->info.remain_hp_b2)
                            {
                                //排除1车状态不是很差但是2车阵亡的情况
                                if(blackboard->info.remain_hp_b1 < 800 
                                    || (blackboard->info.remain_hp_b1 > 800 && blackboard->info.remain_hp_b1 <1800 
                                    && blackboard->info.remain_hp_b2 != 0))
                                    {
                                        getbuff_act = 1;
                                        which_buff = 3;
                                    }//获取血量buff后
                                if(blackboard->info.zone_array_active[blackboard->info.blue_hp_recovery] == 0)
                                {
                                    cur_state = BehaviorStateEnum::WAIT;//等待，与READY夹角
                                }  
                            }
     ...
     /***********************************************************************************************************************/
        else if(blackboard->info.remaining_time <50 && blackboard->info.remaining_time > 0)
        {
            if(blackboard->info.remain_hp_b1 >= 300)
            {        
                getbuff_act = 1;
                which_buff = 3;//直接抢血量
            }
            else cur_state = BehaviorStateEnum::READY;
                if(blackboard->info.zone_array_active[blackboard->info.blue_hp_recovery] == 0 )
                {
                    if((blackboard->info.remain_hp_b1 + blackboard->info.remain_hp_b2) 
                        -(blackboard->info.remain_hp_r1 + blackboard->info.remain_hp_r2) > 500)
                        {
        ...
        else  cur_state = BehaviorStateEnum::PATROL;//全换成巡逻，前面空余时间卡掉

        if (blackboard->info.deep_camera_is_detected && getbuff_act == 0) cur_state = BehaviorStateEnum::SWING;

        if(blackboard->info.remain_hp_b1 < 300) cur_state = BehaviorStateEnum::BACKBOOT;

        if(blackboard->info.zone_array_active[blackboard->info.blue_bullet_supply] == 1 
            && (blackboard->info.remain_bullet_b1<30 || blackboard->info.remain_bullet_b2<30)
            ...
            && blackboard->info.remain_bullet_b1<30 && blackboard->info.remaining_time<165)
            {
                cur_state = BehaviorStateEnum::BACKBOOT;   
            }
        }//getbuff_act == 0
