        ...
        if (blackboard->info.is_begin)
        {
            ...
            if (getbuff_act == 0)        //正式决策树
            {
                if (blackboard->info.remaining_time <= 180 && blackboard->info.remaining_time >= 165)
                {
                    ...
                    if (blackboard->info.blue_bullet_supply == 1 || blackboard->info.blue_bullet_supply == 2)
                    {
                        getbuff_act = 1;
                        which_buff = 4;
                        if (blackboard->info.zone_array_active[blackboard->info.blue_bullet_supply] == 0)  
                    ...
                    else if ((blackboard->info.blue_bullet_supply != 1 && blackboard->info.blue_bullet_supply != 2)
    ...
    //                  }
                ...
                else if (blackboard->info.remaining_time < 150 && blackboard->info.remaining_time >= 120)
                {
                    ...
                    if(((blackboard->info.remain_hp_b1 <= 1500 && blackboard->info.remain_hp_b2 <= 1500) 
                        ...
                        && (blackboard->info.remain_hp_b1 < blackboard->info.remain_hp_b2 && blackboard->info.remain_hp_b1 != 0))
                    {
                        getbuff_act = 1;
                        which_buff = 3;
                        if(blackboard->info.zone_array_active[blackboard->info.blue_hp_recovery] == 0 )  
            ...
            //如果状态较好(或者血量buff已抢，但不满足1)，去巡逻，状态特别好（对面无法行动，则去射击点）
                            ...
                            && blackboard->info.remain_bullet_b2 > 30)    
                {
                    cur_state = BehaviorStateEnum::PATROL;
                    //b1状态特别好，为b1 SHOOT点打掩护
                if(blackboard->info.remain_hp_b1 >= 1900 && blackboard->info.remain_hp_b2 >= 1900 
                    && blackboard->info.remain_bullet_b1> 50)
                    cur_state = BehaviorStateEnum::OBSERVE; //b1 SHOOT掩护点
                }
                else if(blackboard->info.remain_bullet_b2 == 0)
                ...
                else if (blackboard->info.remaining_time < 120 && blackboard->info.remaining_time >= 60)
                {
                            ...
                            {
                                if(blackboard->info.remain_hp_b2 < 800 
                                    || (blackboard->info.remain_hp_b2 > 800 && blackboard->info.remain_hp_b2 <1800
                                    && blackboard->info.remain_hp_b1 != 0))
                                    {
                                        getbuff_act = 1;
                                        which_buff = 3;
                                    }//滤掉1车死亡但是二车血多
     ...
     /***********************************************************************************************************************/
        ...
        {
            if(blackboard->info.remain_hp_b2 < 1300)
            {
                getbuff_act = 1;
                which_buff = 3;//直接抢血量
            }
        ...
        if(blackboard->info.remain_hp_b2 < 300) cur_state = BehaviorStateEnum::PATROL;
