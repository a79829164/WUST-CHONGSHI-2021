    ...
    if(getbuff_act == 0)        //正式决策树
    {
        if(blackboard->info.remaining_time <=180 && blackboard->info.remaining_time > 165)
        {
            ...
            if(blackboard->info.red_bullet_supply == 3 || blackboard->info.red_bullet_supply == 1 
                || blackboard->info.red_bullet_supply == 5 || blackboard->info.red_bullet_supply == 2)
            {
                getbuff_act = 1;
                which_buff = 2;//如果已经激活，被刷掉去下面的行为（ 射击点 ）
                if(blackboard->info.zone_array_active[blackboard->info.red_bullet_supply] == 0)  
                    cur_state = BehaviorStateEnum::SHOOT;
            }
            else if((blackboard->info.red_bullet_supply != 3 && blackboard->info.red_bullet_supply != 1 
                            && blackboard->info.red_bullet_supply != 5  && blackboard->info.red_bullet_supply != 2) 
                            &&  blackboard->info.zone_array_active[blackboard->info.red_bullet_supply] == 0)//2车抢到子弹后
    ...
    //后面巡逻覆盖掉165-150
        ...
        else if(blackboard->info.remaining_time <150 && blackboard->info.remaining_time >= 120)//第一分钟后30秒
        {
            ...
            if(((blackboard->info.remain_hp_r1 <= 1500 && blackboard->info.remain_hp_r2 <= 1500) 
                ...
                && (blackboard->info.remain_hp_r1 > blackboard->info.remain_hp_r2 && blackboard->info.remain_hp_r2 != 0))
            {
                getbuff_act = 1;
                which_buff = 1;
                if(blackboard->info.zone_array_active[blackboard->info.red_hp_recovery] == 0 )  
            ...
            //如果状态较好(或者血量buff已抢，但不满足1)，去巡逻，状态特别好（对面无法行动，则去射击点）
                        ...
                        && blackboard->info.remain_bullet_r1 > 30)    
            {
            ...
            else if(blackboard->info.remain_bullet_r1 == 0)
        ...
        /***********************************************************************************************************************/
            else if(blackboard->info.remaining_time <120 && blackboard->info.remaining_time >= 60)
                        {
                            ...
                            {
                                //排除1车状态不是很差但是2车阵亡的情况
                                if(blackboard->info.remain_hp_r1 < 800 
                                    || (blackboard->info.remain_hp_r1 > 800 && blackboard->info.remain_hp_r1 <1800 
                                    && blackboard->info.remain_hp_r2 != 0))
                                    {
                                        getbuff_act = 1;
                                        which_buff = 1;
                                    }//获取血量buff后
     ...
     /***********************************************************************************************************************/
        ...
        if(blackboard->info.remain_hp_r1 < 300) cur_state = BehaviorStateEnum::BACKBOOT;
        ...
        if(blackboard->info.zone_array_active[blackboard->info.red_bullet_supply] == 1 
           ...
           else if(blackboard->info.zone_array_active[blackboard->info.red_bullet_supply] == 0
            && blackboard->info.remain_bullet_r1<30 && blackboard->info.remaining_time<165)
            {
                cur_state = BehaviorStateEnum::BACKBOOT;   
            }
        }//getbuff_act == 0
